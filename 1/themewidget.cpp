...
QString MainWindow::getLastName(){
 return m_ui->lineEditLastName->text();
  ...
  return m_ui->comboBoxSexe->currentText();
  }

  QString  MainWindow::getDepartement(){
      return  m_ui->lineEditDepartement->text();
  }
  QString MainWindow::getClasse(){
   return m_ui->lineEditClasse->text();

  }

    QString  MainWindow::getSession(){
    return m_ui->lineEditSession->text();
    }

    QString  MainWindow::getAnnee(){
    return m_ui->lineEditDate->text();
    }


void ThemeWidget::reinit() {
        m_ui->lineEditFirstName->setText("");
        m_ui->lineEditLastName->setText("");
        m_ui->comboBoxSexe->setCurrentIndex(0);
        m_ui->lineEditDepartement->setText("");
        m_ui->lineEditClasse->setText("");
       m_ui->lineEditSession->setText("");
        m_ui->lineEditDate->setText("");
