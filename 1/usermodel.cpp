...
User UserModel::read(int id)
{
    ...
    query.prepare("SELECT * FROM t-users WHERE id=:id");
...
bool UserModel::readBy(QString login, QString password)
{
    ...
    query.prepare("SELECT * FROM t-users WHERE login=:login AND password=:password");
...
void UserModel::create(User user)
{
    ...
    query.prepare("INSERT INTO t-users (login, password) VALUES (:login, :password)");
...
void UserModel::update(User user)
{
    ...
    query.prepare("UPDATE t-users SET login=:login, password=:password WHERE id=:id");
...
void UserModel::_delete(int id)
{
    ...
    query.prepare("DELETE FROM t-users WHERE id=:id");
    query.bindValue(":id", id);
...
QList<User> UserModel::list()
{
    ...
    query.prepare("SELECT * FROM t-users WHERE 1");
    query.exec();
